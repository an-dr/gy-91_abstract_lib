...
gy91spi::gy91spi(uint8_t csPin, uint32_t spi_freq)
{
...
void gy91spi::setupMagForSPI()
{
    // Use slave 4 for talking to the magnetometer
    writeByte(49, ((1 << 7) | AK8963_ADDRESS));    // Set the SLV_4_ADDR register to the magnetometer's address
    writeByte(52, 0b00000000);                  // Setup SLV_4 control as needed (but not set to do an operation yet)
    writeByte(36, 0b10000000);   // Enable the multi-master mode
}

void gy91spi::getMres()
{
    switch (Mscale)
    {
...
void gy91spi::readAccelData(int16_t * destination)
{
...
void gy91spi::readGyroData(int16_t * destination)
{
...
void gy91spi::initAK8963(float * destination)
{
    // First extract the factory calibration for each magnetometer axis
    uint8_t rawData[3];  // x/y/z gyro calibration data stored here
    // TODO: Test this!! Likely doesn't work
    writeMagByteSPI(AK8963_CNTL, 0x00); // Power down magnetometer
    delay(10);
    writeMagByteSPI(AK8963_CNTL, 0x0F); // Enter Fuse ROM access mode
    delay(10);
    ...
    writeMagByteSPI(AK8963_CNTL, Mscale << 4 | Mmode);
    delay(10);
...
void gy91spi::calibrateMPU9250(float * gyroBias, float * accelBias)
{
...
// deviation is a pass.
void gy91spi::MPU9250SelfTest(float * destination)
{
...
void gy91spi::magCalMPU9250(float * bias_dest, float * scale_dest)
{
...
uint8_t gy91spi::readMagByteSPI(uint8_t registerAddress)
{
    setupMagForSPI();

    writeByte(49, ((1 << 7) | AK8963_ADDRESS));
    writeByte(50, registerAddress);
    writeByte(52, 0b11000000);        // Command the read into I2C_SLV4_DI register, cause an interrupt when complete

    // Wait for the data to be ready
    uint8_t I2C_MASTER_STATUS = readByte(54);

    uint32_t count = 0;
    while (((I2C_MASTER_STATUS & 0b01000000) == 0) && (count++ < 100000)) // Checks against the I2C_SLV4_DONE bit in the I2C master status register
    {
        I2C_MASTER_STATUS = readByte(54);
    }
    if (count > 10000)
    {
        printf("Timed out\n\r");
    ...
    return readByte(53);   // Read the data that is in the SLV4_DI register
}

uint8_t gy91spi::writeMagByteSPI(uint8_t registerAddress, uint8_t data)
{
    setupMagForSPI();
...
uint8_t gy91spi::ak8963WhoAmI_SPI()
{
    uint8_t response, oldSlaveAddress, oldSlaveRegister, oldSlaveConfig;
    // Save state
    oldSlaveAddress = readByte(I2C_SLV0_ADDR);
    oldSlaveRegister = readByte(I2C_SLV0_REG);
    oldSlaveConfig = readByte(I2C_SLV0_CTRL);
#ifdef SERIAL_DEBUG
    ...
    response = writeByte(I2C_SLV0_ADDR, AK8963_ADDRESS | READ_FLAG);
    // I2C slave 0 register address from where to begin data transfer
    response = writeByte(I2C_SLV0_REG, 0x00);
    // Enable 1-byte reads on slave 0
    response = writeByte(I2C_SLV0_CTRL, 0x81);
//  delayMicroseconds(1); TODO check if it needed
    // Read WIA register
    response = writeByte(WHO_AM_I_AK8963 | READ_FLAG, 0x00);

    // Restore state
    writeByte(I2C_SLV0_ADDR, oldSlaveAddress);
    writeByte(I2C_SLV0_REG, oldSlaveRegister);
    writeByte(I2C_SLV0_CTRL, oldSlaveConfig);
